...
export async function findShifts(dates = DATES, data, adjust = true) {
    ...
    const startDatesWTimes = Array.from(
        new Set(
            shifts
                .filter((shift) =>
                    dates.some(
    ...
    const shiftsMapWTimes = Array.from(
        new Map(
            startDatesWTimes.map((date) => [
                date,
                shifts.filter((shift) => shift.start_date === date)
            ])
        ).values()
    ).flat();
    const startDates = new Map(
        startDatesWTimes.map((date) => [
            getOnlyDate(date),
            shiftsMapWTimes.filter(
                (shift) =>
                    getOnlyDate(date) === getOnlyDate(shift.start_date) &&
                    dates.some(
                        (date) =>
                            date.end_date.getTime() === new Date(shift.end_date).getTime() &&
                            date.end_date.getTime() >= new Date(shift.end_date).getTime()
                    )
            )
        ])
    ...
    const missingDates = DATES.map((x) => getOnlyDate(x.start_date)).filter(
        (x) => !Array.from(startDates.keys()).includes(x)
    );
    if (missingDates.length && adjust) {
        ...
        return Array.from(startDates.values()).concat(
